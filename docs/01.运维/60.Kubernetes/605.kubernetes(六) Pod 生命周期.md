---
title: kubernetes(六) Pod 生命周期
date: 2023-06-25 09:22:36
permalink:  /kubernetes/605
sidebar: true
...
![](/assets/img/kubernetes/605/img.png)

在整个生命周期中，Pod会出现5中状态，分别如下：
* 挂起（Pending）：apiserver已经创建了pod资源对象，但它尚未被调度完成或者仍处于下载镜像的过程中
* 运行中（Running）：pod 已经被调度至某节点，并且所有容器都已经被kubelet创建完成
* 成功（Successded）：pod中的所有容器都已经成功终止并且不会被重启
...
![](/assets/img/kubernetes/605/img_1.png)
...
假设要以主容器来运行nginx，但是要求在运行nginx之前先要能够连接上相应的服务器是可以ping通的。
