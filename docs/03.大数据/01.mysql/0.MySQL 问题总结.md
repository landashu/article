...
所谓行大小限制，就是你给字段设置的大小，比如a字段为varchar(30),那么他就占总行的30字节，所有字段所占字节的总和超过限制则会报错。
这种情况可以给某些字段设置TEXT或BLOB，这样这些字段的数据将不再保存在行中，而是保存在数据库中，行大小将不会受限制，也就是若数据较短(<768字节在行内)，若数据较长(>768字节在行外，即行内存的是指针占20个字节)，
